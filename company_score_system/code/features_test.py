# -*- coding: utf-8 -*-
"""
Created on Sat Apr 20 23:02:36 2019
...
#%%
data=np.arange(4001,4525)
df_test= pd.DataFrame(data,columns=['企业编号'])
#%%
develop=load_data('赛题1测试数据集/上市信息财务信息-成长能力指标.xlsx')
...
#%%
operate=load_data('赛题1测试数据集/上市信息财务信息运营能力指标.xlsx')
...
#%%
stock_index=load_data('赛题1测试数据集/上市公司财务信息-每股指标.xlsx')
...
#%%
risk_index=load_data('赛题1测试数据集/上市信息财务信息-财务风险指标.xlsx')
...
#%%
profitability_index=load_data('赛题1测试数据集/上市信息财务信息盈利能力指标.xlsx')
...
#%%
products=load_data('赛题1测试数据集/产品.xlsx')
new_products=products.groupby('企业编号',as_index=False).count()
new_products=pd.merge(df_test,new_products,on='企业编号',how='outer')
new_products.fillna(0,inplace=True)
#%%
arank_year=load_data('赛题1测试数据集/纳税A级年份.xlsx')
arank_year=arank_year.groupby('企业编号',as_index=False).count()
new_arank=pd.merge(df_test,arank_year,on='企业编号',how='outer')
new_arank.fillna(0,inplace=True)
#%%工商基本信息表
infor=load_data('赛题1测试数据集/工商基本信息表.xlsx')
...
#%%
land_mortgage=load_data('赛题1测试数据集/购地-市场交易-土地抵押.xlsx') 
...
new_land_mortgage=pd.merge(df_test,new_land_mortgage,on='企业编号',how='outer')
new_land_mortgage.fillna(0,inplace=True)
new_land_mortgage['是否购地']=new_land_mortgage['土地面积'].apply(lambda x: 'Yes' if x > 0 else 'No')
land_use_table=pd.read_excel('赛题1测试数据集/土地使用表.xlsx',dtype='object')
land_use_dict={}
...
#%%
land_transfer=load_data('赛题1测试数据集/购地-市场交易-土地转让.xlsx')
...
#%%
compete_goods=load_data('赛题1测试数据集/竞品.xlsx')
...
new_software_copyrights=pd.merge(df_test,new_software_copyrights,on='企业编号',how='outer')
...
#%%
annual_web=load_data('赛题1测试数据集/年报-网站或网点信息.xlsx')
new_annual_web=annual_web.groupby('企业编号',as_index=False).count()
new_annual_web=pd.merge(df_test,new_annual_web,on='企业编号',how='outer')
new_annual_web.fillna(0,inplace=True)

#%%
annual_infor=load_data('赛题1测试数据集/年报-股东（发起人）及出资信息.xlsx')
new_annual_infor=annual_infor.groupby('企业编号',as_index=False).count()
new_annual_infor=pd.merge(df_test,new_annual_infor,on='企业编号',how='outer')
new_annual_infor.fillna(0,inplace=True)
new_annual_infor['是否认缴']=new_annual_infor['实缴出资信息'].apply(lambda x: 'Yes' if x > 0 else 'No')
#%%
test_num_features=[new_develop,new_operate_index,new_risk_index,\
...
#%%
data=np.arange(4001,4525)
test_num_data = pd.DataFrame(data,columns=['企业编号'])
#%%
for i in range(len(test_num_features)):
    test_num_data=pd.merge(test_num_data,test_num_features[i],on='企业编号',how='outer')
#%%
company_index=load_data('企业编号.xlsx')
test_num_data=pd.merge(company_index,test_num_data,on='企业编号',how='inner')
test_num_data.to_csv("data/test_num0422.csv",index=False,na_rep="NULL",encoding='utf_8_sig')
#%%
remain_bond.to_csv('data/test_remain_bond0422.csv',index=False,na_rep="NULL",encoding='utf_8_sig')
remain_compete.to_csv('data/test_remain_compete0422.csv',index=False,na_rep="NULL",encoding='utf_8_sig')
remain_credit.to_csv('data/test_remain_credit0422.csv',index=False,na_rep="NULL",encoding='utf_8_sig')
remain_land.to_csv('data/test_remain_land0422.csv',index=False,na_rep="NULL",encoding='utf_8_sig')
remain_finance.to_csv('data/test_remain_finance0422.csv',index=False,na_rep="NULL",encoding='utf_8_sig')
